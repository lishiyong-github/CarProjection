...
- (void)cellDidLoad {
    ...
    [self.contentView addSubview:self.coBackButton];
...
- (void)updateConstraints {
    if (!self.didSetupConstraints) {
        ...
        [self.coBackButton autoPinEdgesToSuperviewEdgesWithInsets:UIEdgeInsetsMake(0, 7.5, 0, 7.5)];
        
        [self.carImageView autoPinEdge:ALEdgeTop toEdge:ALEdgeTop ofView:self.coBackButton withOffset:7.5];
        [self.carImageView autoPinEdge:ALEdgeLeft toEdge:ALEdgeLeft ofView:self.coBackButton withOffset:7.5];
        [self.carImageView autoPinEdge:ALEdgeBottom toEdge:ALEdgeBottom ofView:self.coBackButton withOffset:-7.5];
        [self.carImageView autoSetDimension:ALDimensionWidth toSize:100];
        
        [self.nameLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeTop ofView:self.carImageView withOffset:7.5];
        ...
        [self.lisenceLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:self.nameLabel withOffset:bigSpacing];
        
        [self.qualityLabel autoPinEdge:ALEdgeLeft toEdge:ALEdgeLeft ofView:self.lisenceLabel];
        [self.qualityLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:self.lisenceLabel withOffset:bigSpacing];
        [self.qualityLabel autoSetDimensionsToSize:CGSizeMake(50, 15)];
        
        [self.priceLabel autoPinEdge:ALEdgeLeft toEdge:ALEdgeLeft ofView:self.qualityLabel];
        [self.priceLabel autoPinEdge:ALEdgeBottom toEdge:ALEdgeBottom ofView:self.coBackButton withOffset:-7.5];
        
        [self.countLabel autoPinEdge:ALEdgeBottom toEdge:ALEdgeBottom ofView:self.priceLabel];
        [self.countLabel autoPinEdge:ALEdgeRight toEdge:ALEdgeRight ofView:self.coBackButton withOffset:-middleSpacing];
        
//        [self.cancelButton autoAlignAxis:ALAxisHorizontal toSameAxisOfView:self.nameLabel];
...
- (UIButton *)coBackButton {
    if (!_coBackButton) {
        _coBackButton = [UIButton newAutoLayoutView];
        _coBackButton.backgroundColor = MLWhiteColor;
    }
    return _coBackButton;
}
...
- (UILabel *)lisenceLabel {
    if (!_lisenceLabel) {
        ...
        _lisenceLabel.textColor = MLBlackColor;
...
- (UILabel *)qualityLabel {
    if (!_qualityLabel) {
        ...
        _qualityLabel.textColor = MLGrayColor;
...
- (void)cellWillAppear {
    ...
    self.lisenceLabel.text = self.item.collocation;
    self.qualityLabel.text = @"精选优车";
